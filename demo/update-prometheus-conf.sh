  ...
  prometheus.yml: |
    ...
    scrape_configs:
      ...
      - job_name: 'kubernetes-cadvisor'
        ...
        metric_relabel_configs:
        - source_labels: [__name__]
          regex: '(container_cpu_usage_seconds_total|container_memory_rss|container_memory_usage_bytes|container_network_receive_bytes_total)'
          action: keep
        relabel_configs:
        - action: labelmap
          regex: __meta_kubernetes_node_label_(.+)
...
metadata:
  annotations:
    meta.helm.sh/release-name: $MESH_NAME
    meta.helm.sh/release-namespace: $K8S_NAMESPACE
  labels:
    app.kubernetes.io/instance: $MESH_NAME
...
kubectl delete po -l app=osm-prometheus -n $K8S_NAMESPACE